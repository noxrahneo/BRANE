...
def main() -> int:
    ...
    #load pseudobulk logcpm, metadata, and qc tables
    ...
    if not logcpm_file.exists():
        raise FileNotFoundError(f"Missing all-conditions logCPM matrix: {logcpm_file}")
    ...
    #align ids between metadata and logcpm columns
    ...
    #merge qc metrics into metadata
    qc_cols = ["pseudobulk_id", "library_size", "detected_genes", "counts_match_meta", "low_n_cells_flag", "profile_outlier_flag"]
    ...
    #build flagged and include masks
    flagged_ids = set(qc_flagged["pseudobulk_id"].astype(str).tolist()) if "pseudobulk_id" in qc_flagged.columns else set()
    flagged_mask = pd.DataFrame({"pseudobulk_id": ids, "is_flagged": [pid in flagged_ids for pid in ids]})

    include_all = pd.DataFrame({"pseudobulk_id": ids, "include_in_network": True})
    include_excl = pd.DataFrame({"pseudobulk_id": ids, "include_in_network": ~flagged_mask["is_flagged"].values})

    #build pack summary and sparsity diagnostics
    summary = pd.DataFrame([{
        ...
        "n_conditions": int(meta_qc["condition"].astype(str).nunique()) if "condition" in meta_qc.columns else None,
    }])

    values = aligned_logcpm.to_numpy(dtype=float)
    sparsity_diag = pd.DataFrame([{
        "n_genes": int(aligned_logcpm.shape[0]),
        "n_profiles": int(aligned_logcpm.shape[1]),
        "matrix_size": int(values.size),
        "n_nonfinite": int(values.size - np.isfinite(values).sum()),
        "n_negative": int(np.sum(values < 0)),
        "zero_fraction": float(np.mean(np.abs(values) <= 1e-12)),
        "lt_0p01_fraction": float(np.mean(values < 0.01)),
        "lt_0p1_fraction": float(np.mean(values < 0.1)),
    }])

    #save all pack outputs
    ...
    sparsity_diag.to_csv(out_dir / "combined_logcpm_sparsity_diagnostics.csv", index=False)
