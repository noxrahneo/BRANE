#!/usr/bin/env python3
"""Consolidate cell-type and cancer gene annotations into a master node table per (gene, pair)."""
...
#pyvis optional; used if full html regeneration is needed
try:
...
def validate_condition_folders(annotation_dir: Path, conditions: set[str]) -> dict[str, bool]:
    ...
    for cond in sorted(conditions):
        ...
        status[cond] = exists
        if not exists:
            print(f"[warn]: Condition folder not found: {cond_dir}")
    return status
...
def load_immune_markers(immune_file: Path) -> dict[str, str]:
    ...
    if not immune_file.exists():
        print(f"[warn]: Immune markers file not found: {immune_file}")
        return {}
    ...
    try:
        ...
        print(f"[warn]: Failed to load immune markers: {e}")
        return {}
...
def load_condition_annotation(
    ...
    annotations = immune_markers.copy()  #start with immune markers as baseline
    ...
    if cluster_markers_file.exists() and cluster_mapping_file.exists():
        try:
            ...
            print(f"[warn]: Failed to load cluster markers for {cond}: {e}")
    ...
    if zscore_file.exists():
        try:
            ...
            print(f"[warn]: Failed to load zscore file for {cond}: {e}")
...
def update_html_node_data(
    ...
    html_content = html_content.replace(
        "</head>",
        f"""<!-- cell-type metadata injected by 34_node_annotation_assembly.py -->
...
def regenerate_network_visualizations(
    ...
    for viz_dir_str in network_viz_dirs:
        ...
        if not viz_dir.exists():
            print(f"[warn]: Viz dir not found: {viz_dir}")
            continue
    ...
    for html_file in html_files:
        ...
        if not pair_name:
            print(f"[skip]: Cannot extract pair_name from {html_file}")
            continue
        
        #filter master table for this pair
        pair_df = master_df[master_df["pair_name"] == pair_name]
        if pair_df.empty:
            print(f"[skip]: No data found for pair {pair_name}")
            continue
        ...
        if updated_html:
            ...
            print(f"[skip]: Failed to update HTML for {pair_name}")
